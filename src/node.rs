...
impl<K: KeyTrait + Clone, V: Clone> TwigNode<K, V> {
    ...
    pub(crate) fn get_leaf_by_query_ref(
        ...
        match query_type {
            ...
            QueryType::LastLessThanVersion(ts) => self.last_less_than_version(ts),
            QueryType::LastLessOrEqualVersion(ts) => self.last_less_or_equal_version(ts),
            QueryType::FirstGreaterThanVersion(ts) => self.first_greater_than_version(ts),
            QueryType::FirstGreaterOrEqualVersion(ts) => self.first_greater_or_equal_version(ts),
        }
    }
    ...
    #[inline]
    pub fn get_all_versions(&self) -> Vec<(V, u64)> {
        self.values
            .iter()
            .map(|value| (value.value.clone(), value.version))
            .collect()
    }

    #[inline]
    pub fn last_less_than_version(&self, version: u64) -> Option<&Arc<LeafValue<V>>> {
        self.values
            .iter()
            .filter(|value| value.version < version)
            .max_by_key(|value| value.version)
    ...
    #[inline]
    pub fn last_less_or_equal_version(&self, version: u64) -> Option<&Arc<LeafValue<V>>> {
        self.get_leaf_by_version(version)
    }

    #[inline]
    pub fn first_greater_than_version(&self, version: u64) -> Option<&Arc<LeafValue<V>>> {
        self.values
            .iter()
            .filter(|value| value.version > version)
            .min_by_key(|value| value.version)
    }

    #[inline]
    pub fn first_greater_or_equal_version(&self, version: u64) -> Option<&Arc<LeafValue<V>>> {
        self.values
            .iter()
            .filter(|value| value.version >= version)
            .min_by_key(|value| value.version)
...
mod tests {
    ...
    #[test]
    fn twig_get_leaf_by_version_2() {
        let dummy_prefix: FixedSizeKey<8> = FixedSizeKey::create_key("bar".as_bytes());
        let mut node = TwigNode::<FixedSizeKey<8>, usize>::new(dummy_prefix.clone(), dummy_prefix);
        // Inserting leaves with different timestamps
        node.insert_mut(50, 200); // value: 50, version: 200
        node.insert_mut(51, 201); // value: 51, version: 201

        // Test case 1: Retrieve leaf by exact timestamp
        let leaf_by_version = node.get_leaf_by_version(10);
        assert_eq!(leaf_by_version.unwrap().value, 50);

        // Test case 2: Retrieve leaf by another exact timestamp
        let leaf_by_version = node.get_leaf_by_version(20);
        assert_eq!(leaf_by_version.unwrap().value, 51);

        // Test case 3: Attempt to retrieve leaf by a non-existent timestamp
        let leaf_by_version = node.get_leaf_by_version(30);
        assert_eq!(leaf_by_version.unwrap().value, 51);
    }
    ...
    fn test_get_all_versions() {
        ...
        assert_eq!(versions[0], (50, 200));
        assert_eq!(versions[1], (51, 201));
    }
    ...
    fn test_last_less_than_version() {
        ...
        let leaf = node.last_less_than_version(20);
        assert_eq!(leaf.unwrap().value, 50);

        // Timestamp well below any existing timestamp
        let leaf = node.last_less_than_version(5);
        assert!(leaf.is_none());

        // Timestamp above all existing timestamps
        let leaf = node.last_less_than_version(25);
        assert_eq!(leaf.unwrap().value, 51);
    }
    ...
    fn test_last_less_or_equal_version() {
        ...
        // Exact timestamp
        let leaf = node.last_less_or_equal_version(10);
        assert_eq!(leaf.unwrap().value, 50);

        // Timestamp not present, should get closest lower timestamp
        let leaf = node.last_less_or_equal_version(15);
        assert_eq!(leaf.unwrap().value, 50);

        // Higher timestamp, should get the highest available timestamp
        let leaf = node.last_less_or_equal_version(25);
        assert_eq!(leaf.unwrap().value, 51);
    }
    ...
    fn test_first_greater_than_version() {
        ...
        let leaf = node.first_greater_than_version(10);
        assert_eq!(leaf.unwrap().value, 51);

        // Timestamp well above any existing timestamp
        let leaf = node.first_greater_than_version(25);
        assert!(leaf.is_none());

        // Timestamp below all existing timestamps
        let leaf = node.first_greater_than_version(5);
        assert_eq!(leaf.unwrap().value, 50);
    }
    ...
    fn test_first_greater_or_equal_version() {
        ...
        // Exact timestamp
        let leaf = node.first_greater_or_equal_version(10);
        assert_eq!(leaf.unwrap().value, 50);

        // Timestamp not present, should get closest higher timestamp
        let leaf = node.first_greater_or_equal_version(15);
        assert_eq!(leaf.unwrap().value, 51);

        // Lower timestamp, should get the lowest available timestamp
        let leaf = node.first_greater_or_equal_version(5);
        assert_eq!(leaf.unwrap().value, 50);
    }
}
