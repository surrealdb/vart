...
impl<'a, P: PrefixTrait, V: Clone> SnapshotPointer<'a, P, V> {
    ...
    pub fn ts(&self) -> Result<u64, TrieError> {
        ...
        // Try to get the snapshot at the specified index, returning `SnapshotNotFound` if not found
    ...
    pub fn get<K: Key>(&self, key: &K) -> Result<Option<(V, u64)>, TrieError> {
        ...
        // Try to get the snapshot at the specified index, returning `SnapshotNotFound` if not found
        let snapshot = snapshots
            .get(self.index).ok_or(TrieError::SnapshotNotFound)?;
    ...
    pub fn close(&mut self) -> Result<(), TrieError> {
        // Call the close method of the Tree with the index of the snapshot to close it
...
mod tests {
    ...
    fn test_snapshot_isolation() {
        ...
        assert_eq!(snap1.index, 0);
        assert_eq!(snap2.index, 1);

        // keys inserted before snapshot creation should be visible
        assert_eq!(snap1.get(&VectorKey::from_str("key_1")).unwrap().unwrap(), (1, 1));
        assert_eq!(snap2.get(&VectorKey::from_str("key_1")).unwrap().unwrap(), (1, 1));

        // keys inserted after snapshot creation should not be visible to other snapshots
        assert!(snap1.insert(&VectorKey::from_str("key_1_snap1"), 2).is_ok());
        assert!(snap2.insert(&VectorKey::from_str("key_1_snap2"), 3).is_ok());

        assert_eq!(snap1.get(&VectorKey::from_str("key_1_snap1")).unwrap().unwrap(), (2, 2));
        assert!(snap1.get(&VectorKey::from_str("key_1_snap2")).unwrap().is_none());
        assert_eq!(snap2.get(&VectorKey::from_str("key_1_snap2")).unwrap().unwrap(), (3, 2));
        assert!(snap2.get(&VectorKey::from_str("key_1_snap1")).unwrap().is_none());
