...
impl<P: KeyTrait, V: Clone> Snapshot<P, V> {
    ...
    pub fn insert(&mut self, key: &P, value: V) {
        // Insert the key-value pair into the root node using a recursive function
        match &self.root {
            Some(root) => {
                let (new_node, _) = Node::insert_recurse(root, key, value, self.version, 0);
    ...
    pub fn get(&self, key: &P) -> Option<(V, u64)> {
    ...
    /// Retrieves the value, version associated with the given key from the snapshot.
    pub fn get_at_version(&self, key: &P, version: u64) -> Option<(V, u64)> {
        self.root
            .as_ref()
            .and_then(|root| Node::get_recurse(root, key, QueryType::LatestByVersion(version)))
    ...
    pub fn scan_at_version<R>(&self, range: R, version: u64) -> Vec<(Vec<u8>, V)>
    where
        R: RangeBounds<P>,
    {
        scan_node(self.root.as_ref(), range, QueryType::LatestByVersion(version))
    }

    pub fn keys_at_version<R>(&self, range: R, version: u64) -> Vec<Vec<u8>>
    where
        R: RangeBounds<P>,
    {
        query_keys_at_node(self.root.as_ref(), range, QueryType::LatestByVersion(version))
    }
}
...
mod tests {
    ...
    fn snapshot_creation() {
        ...
        for key in keys.iter() {
            assert!(tree
                .insert(&VariableSizeKey::from_str(key).unwrap(), 1, 0)
                .is_ok());
        }

        let mut snap1 = tree.create_snapshot();
        let key_to_insert = "key_1";
        snap1.insert(&VariableSizeKey::from_str(key_to_insert).unwrap(), 1);

        let expected_snap_version = keys.len() as u64 + 1;
        assert_eq!(snap1.version(), expected_snap_version);

        let expected_tree_version = keys.len() as u64;
        assert_eq!(tree.version(), expected_tree_version);
    }
    ...
    fn snapshot_isolation() {
        ...
        let mut snap1 = tree.create_snapshot();
        assert_eq!(snap1.get(&key_1).unwrap(), (1, 1));

        let mut snap2 = tree.create_snapshot();
        assert_eq!(snap2.get(&key_1).unwrap(), (1, 1));
        ...
        snap1.insert(&key_3_snap1, 2);
        assert_eq!(snap1.get(&key_3_snap1).unwrap(), (2, 2));

        snap2.insert(&key_3_snap2, 3);
        assert_eq!(snap2.get(&key_3_snap2).unwrap(), (3, 2));
    ...
    #[test]
    fn test_get_at_version_successful_retrieval() {
        ...
        let version = 1;
        let (retrieved_value, retrieved_version) = snapshot.get_at_version(&key, version).unwrap();
        assert_eq!(retrieved_value, value);
        assert_eq!(retrieved_version, 1);
    }

    #[test]
    fn test_get_at_version_key_not_found() {
        let tree: Tree<VariableSizeKey, i32> = Tree::<VariableSizeKey, i32>::new();
        let snapshot = tree.create_snapshot();
        let key = VariableSizeKey::from_str("nonexistent_key").unwrap();
        let result = snapshot.get_at_version(&key, 100);
        assert!(result.is_none());
    }

    #[test]
    fn test_get_at_version_timestamp_before_insertion() {
        let tree: Tree<VariableSizeKey, i32> = Tree::<VariableSizeKey, i32>::new();
        let mut snapshot = tree.create_snapshot();
        let key = VariableSizeKey::from_str("test_key").unwrap();
        let value = 10;
        ...
        let result = snapshot.get_at_version(&key, ver - 1);
        assert!(result.is_none());
    }
    ...
    //     let value1 = 10;
    //     let value2 = 11;
    //     snapshot.insert(&key, value1);
    //     snapshot.insert(&key, value2);

    //     let (retrieved_value, _) = snapshot.get_at_version(&key, ts2).unwrap();
    ...
    //     let (retrieved_value, _) = snapshot.get_at_version(&key, ts_query).unwrap();
    ...
    //     let (retrieved_value, _) = snapshot.get_at_version(&key, ts).unwrap();
    //     assert_eq!(retrieved_value, value2);
    // }
    ...
    //     let (retrieved_value, _) = snapshot.get_at_version(&key, ts_future).unwrap();
    //     assert_eq!(retrieved_value, value);
    // }
    ...
    //     let (retrieved_value_early, _) = snapshot.get_at_version(&key, ts2).unwrap();
    //     let (retrieved_value_late, _) = snapshot.get_at_version(&key, ts1).unwrap();
    ...
    //     let result = snapshot.get_at_version(&key, ts_query);
    //     assert!(result.is_none());
    // }
    ...
    // #[test]
    // fn test_last_less_than_version() {
    //     let tree: Tree<VariableSizeKey, i32> = Tree::new();
    //     let mut snapshot = tree.create_snapshot();
    //     let key = VariableSizeKey::from_str("test_key").unwrap();
    ...
    // #[test]
    // fn test_last_less_or_equal_version() {
    //     let tree: Tree<VariableSizeKey, i32> = Tree::new();
    //     let mut snapshot = tree.create_snapshot();
    //     let key = VariableSizeKey::from_str("test_key").unwrap();
    //     snapshot.insert(&key, 10, 100);
    ...
    // #[test]
    // fn test_first_greater_than_version() {
    //     let tree: Tree<VariableSizeKey, i32> = Tree::new();
    //     let mut snapshot = tree.create_snapshot();
    //     let key = VariableSizeKey::from_str("test_key").unwrap();
    //     snapshot.insert(&key, 10, 100);
    ...
    // #[test]
    // fn test_first_greater_or_equal_version() {
    //     let tree: Tree<VariableSizeKey, i32> = Tree::new();
    //     let mut snapshot = tree.create_snapshot();
    //     let key = VariableSizeKey::from_str("test_key").unwrap();
    //     snapshot.insert(&key, 10, 100);
    ...
    fn scan_empty_range() {
        ...
        let result = snap.scan_at_version(RangeFull {}, 0);
    ...
    fn scan_range_includes_some_keys() {
        ...
        let keys = ["key_1", "key_2", "key_3"];
        for key in keys.iter() {
            tree.insert(&VariableSizeKey::from_str(key).unwrap(), 1, 0)
                .unwrap();
        }
        let snap = tree.create_snapshot();
        ...
        let values = snap.scan_at_version(range, 0);
    ...
    fn scan_range_includes_all_keys() {
        ...
        let snap = tree.create_snapshot();
        let values = snap.scan_at_version(RangeFull {}, 0);
    ...
    fn scan_at_different_versions() {
        ...
        for (i, _) in keys.iter().enumerate() {
            let values = snap.scan_at_version(RangeFull {}, i as u64);
    ...
    fn large_number_of_inserts() {
        ...
        for i in 0..num_keys {
            ...
            tree.insert(&VariableSizeKey::from_str(&key).unwrap(), i, 0)
                .unwrap();
        }

        let snap = tree.create_snapshot();
        let values = snap.scan_at_version(RangeFull {}, num_keys);
        assert_eq!(values.len(), num_keys as usize); // Expect all keys to be visible
    }

    #[test]
    fn scan_at_various_versions() {
        let mut tree: Tree<VariableSizeKey, i32> = Tree::new();
        let keys = ["key_1", "key_2", "key_3"];
        for (i, key) in keys.iter().enumerate() {
        ...
        let result_before = snap.scan_at_version(RangeFull {}, 0);
        assert!(result_before.is_empty());

        // Scan between insertions
        let result_mid = snap.scan_at_version(RangeFull {}, 4);
        assert_eq!(result_mid.len(), 2); // Expect first two keys to be visible

        // Scan after all insertions
        let result_after = snap.scan_at_version(RangeFull {}, 7);
    ...
    fn scan_with_single_item_in_snapshot() {
        ...
        tree.insert(&VariableSizeKey::from_str("key_1").unwrap(), 42, 0)
            .unwrap();
    ...
    fn keys_at_empty_range() {
        let tree: Tree<VariableSizeKey, i32> = Tree::new();
        let snap = tree.create_snapshot();
        let keys = snap.keys_at_version(RangeFull {}, 0);
    ...
    fn keys_at_range_includes_some_keys() {
        ...
        let range = VariableSizeKey::from_slice_with_termination("key_1".as_bytes())
            ..=VariableSizeKey::from_slice_with_termination("key_2".as_bytes());
        let keys = snap.keys_at_version(range, 0);
    ...
    fn keys_at_different_timestamps() {
        ...
        for (i, _) in keys_to_insert.iter().enumerate() {
            let keys = snap.keys_at_version(RangeFull {}, i as u64);
    ...
    fn keys_at_large_number_of_inserts() {
        ...
        let snap = tree.create_snapshot();
        let keys = snap.keys_at_version(RangeFull {}, num_keys);
    ...
    fn keys_at_various_timestamps() {
        ...
        let snap = tree.create_snapshot();
        let keys_before = snap.keys_at_version(RangeFull {}, 0);
        assert!(keys_before.is_empty());

        let keys_mid = snap.keys_at_version(RangeFull {}, 4);
        assert_eq!(keys_mid.len(), 2); // Expect first two keys to be visible

        let keys_after = snap.keys_at_version(RangeFull {}, 7);
